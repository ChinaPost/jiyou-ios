...
@implementation DropDownForOrderForm
...

{
    ...
    NSString *msg=((DropDownRowForOrderForm*)tableArray[indexPath.row]).rowMsg;
    NSString *price=((DropDownRowForOrderForm*)tableArray[indexPath.row]).price;
    
    if(price==nil)
        price=@"";
    
    cell.textLabel.text =[NSString stringWithFormat:@"%@      %@",msg,price];
    cell.textLabel.tag=[((DropDownRowForOrderForm*)[tableArray objectAtIndex:[indexPath row]]).rowId integerValue];
...

{
     ...
     code=((DropDownRowForOrderForm*)tableArray[indexPath.row]).rowId;
    NSString *msg=((DropDownRowForOrderForm*)tableArray[indexPath.row]).rowMsg;
    NSString *price=((DropDownRowForOrderForm*)tableArray[indexPath.row]).price;
    if(price==nil)
        price=@"";
...
@implementation DropDownRowForOrderForm
